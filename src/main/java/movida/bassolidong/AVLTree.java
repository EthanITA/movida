...
public class AVLTree {
	public class AVLNode {
		...
		public AVLNode(Movie m){this.m=m;}
		public AVLNode getLeft() {return left;}
		public AVLNode getRight() {return right;}
		public String getTitle() {return this.m.getTitle();}
	}
	...
	// get height
	public int height_tree() {return height_tree(root);}
	...
	public void makeEmpty() {
		...
		public boolean contains(Movie m) {return contains(root,m);}
		public boolean contains(AVLNode node, Movie m) {
			if(node == null) {return false;}
			
			int compare=m.getTitle().compareTo(node.getTitle());
			
			if(compare < 0) {return contains(node.left,m);}
			if(compare > 0) {return contains(node.right,m);}
	    ...
	    public boolean insert(Movie m) {
	    	if(m == null || m.getTitle() == null) {return false;}
	    ...
	    AVLNode insert(AVLNode node, Movie m) { 
			...
			if(compare < 0) {node.left = insert(node.left,m);}
			else {node.right = insert (node.right,m);}
		...
		{
			if(node == null || node.getTitle()== null) {return null;}
			
			int compare= m.getTitle() .compareTo ( node.getTitle() );
			
			if(compare < 0) {node.left = remove(node.left,m);}
			else if(compare > 0) {node.right= remove(node.right,m);}
		...
		public Movie getMovieByTitle(String Title) {return getMovieByTitle(root,Title);}
		public Movie getMovieByTitle(AVLNode node,String Title)
		{
			...
			if(compare < 0) {return getMovieByTitle(node.left,Title);}
			if(compare > 0) {return getMovieByTitle(node.right,Title);}
			if(compare == 0) {return node.m;}
